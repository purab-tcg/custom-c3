...
(function (global, factory) {
    ...
    var CLASS = {
        ...
        selectedPoint: 'c3-selected-circle',
        selectedPoints: 'c3-selected-circles',
    ...
    ChartInternal.prototype.redraw = function (options, transitions) {
        ...
        // update pointY based on updated parameters
        $$.updatePointY();
        // generate circle x/y functions depending on updated params
        cx = ($$.config.axis_rotated ? $$.pointY : $$.pointX).bind($$);
        cy = ($$.config.axis_rotated ? $$.pointX : $$.pointY).bind($$);
        // Update sub domain
        if (withY) {
        ...
        // circles for select
        main
            .selectAll('.' + CLASS.selectedPoints)
        ...
        if (duration && $$.isTabVisible()) {
            ...
            [
                ...
                $$.redrawPointShape(cx, cy, true, transition),
    ...
    ChartInternal.prototype.initialOpacityForPoints = function (d) {
        return d.value !== null && this.withoutFadeIn[d.id]
            ? this.opacityForPoints(d)
            : 0;
    };
    ChartInternal.prototype.opacityForPoints = function (d) {
    ...
    ChartInternal.prototype.getDefaultConfig = function () {
        var config = {
            ...
            legend_padding: 0,
            legend_item_tile_width: 40,
            legend_item_tile_height: 20,
            legend_item_tile_showLine: true,
            legend_item_tile_dotSize: 4,
            // axis
            axis_rotated: false,
            axis_x_show: true,
            axis_x_type: 'indexed',
    ...
    ChartInternal.prototype.dist = function (data, pos) {
        var $$ = this, config = $$.config, xIndex = config.axis_rotated ? 1 : 0, yIndex = config.axis_rotated ? 0 : 1, y = $$.pointY(data, data.index), x = $$.x(data.x);
    ...
    ChartInternal.prototype.getXDomainPadding = function (domain) {
        ...
        if ($$.isCategorized()) {
            ...
            padding = diff * config.axis_x_margin / 100;
        }
                ...
                ? function () {
                    ...
                    if (config.point_focus_expand_enabled) {
                        ...
                        selectedData.forEach(function (d) {
                            $$.expandPointShapes(d.index, d.id, false);
                        });
                    }
                    // expand bars
                    $$.unexpandBars();
    ...
    ChartInternal.prototype.updateLegend = function (targetIds, options, transitions) {
        ...
        l.append('path')
            ...
            .style('pointer-events', 'none')
            .attr('d', function (d) {
                var x1 = $$.isLegendRight || $$.isLegendInset ? x1ForLegendTile(d) : -200;
                var x2 = $$.isLegendRight || $$.isLegendInset ? x2ForLegendTile(d) : -200;
                var y = $$.isLegendRight || $$.isLegendInset ? -200 : yForLegendTile(d);
                var x = (x1 + x2) / 2;
                var legend = $$.legendShape(d);
                return $$.getDotPathPoints(x, y, legend.r, legend.shape);
        ...
        l.append('line')
            ...
            .style('stroke-dasharray', function (d) {
                return $$.lineStyle({ id: d });
            })
            .style('visibility', function (d) {
                if (!$$.config.legend_item_tile_showLine) {
                    return 'hidden';
                }
                return $$.isBarType(d) || $$.isArcType(d) ? 'hidden' : '';
            });
        ...
        tiles = $$.legend.selectAll('path.' + CLASS.legendItemTile).data(targetIds);
        (withTransition ? tiles.transition() : tiles)
            .style('fill', $$.color)
            .style('shape-rendering', 'auto')
            .attr('d', function (d) {
                var x = (x1ForLegendTile(d) + x2ForLegendTile(d)) / 2;
                var y = yForLegendTile(d);
                var legend = $$.legendShape(d);
                return $$.getDotPathPoints(x, y, legend.r, legend.shape);
            }).style('stroke-width', '2px')

        var tiles2 = $$.legend.selectAll('line.' + CLASS.legendItemTile).data(targetIds);
        (withTransition ? tiles2.transition() : tiles2)
    ...
    ChartInternal.prototype.selectPoint = function (target, d, i) {
        //console.log('Okay');
        var $$ = this, config = $$.config, cx = (config.axis_rotated ? $$.pointY : $$.pointX).bind($$), cy = (config.axis_rotated ? $$.pointX : $$.pointY).bind($$), r = $$.pointSelectR.bind($$);
        config.data_onselected.call($$.api, d, target.node());
        // add selected-circle on low layer g
        $$.main
            .select('.' + CLASS.selectedPoints + $$.getTargetSelectorSuffix(d.id))
            .selectAll('.' + CLASS.selectedPoint + '-' + i)
            .data([d])
            .enter()
            .append('circle')
            .attr('class', function () {
                return $$.generateClass(CLASS.selectedPoint, i);
            })
            .attr('cx', cx)
            .attr('cy', cy)
    ...
    ChartInternal.prototype.getToggle = function (that, d) {
        ...
        if (that.nodeName === 'circle' || that.nodeName === 'polygon') {
            ...
            if (d3.select(that).classed(CLASS.shape)) {
                // sandeep
                toggle = $$.togglePoint;
            } else {
                toggle = $$.togglePath;
    ...
    ChartInternal.prototype.isWithinShape = function (that, d) {
        ...
        //console.log(shape.classed(CLASS.shape));
        if (!$$.isTargetToShow(d.id)) {
            isWithin = false;
        }
        else if (shape.classed(CLASS.shape)) {
            //sandeep
    ...
    ChartInternal.prototype.updateTargetsForLine = function (targets) {
        ...
        mainLineEnter.append('g').attr('class', function (d) {
            return $$.generateClass(CLASS.selectedPoints, d.id);
        });
        mainLineEnter
            .append('g')
        ...
        targets.forEach(function (t) {
            $$.main
                .selectAll('.' + CLASS.selectedPoints + $$.getTargetSelectorSuffix(t.id))
                .selectAll('.' + CLASS.selectedPoint)
                .each(function (d) {
    ...
    ChartInternal.prototype.getDotPathPoints = function (x, y, r, dotShape) {
        if (!dotShape || !x || !r || !y) {
            return;
        }
        if (dotShape === 'cross') {
            r = 1.6 * r;
            return 'M' + (x - r * 0.707) + ' ' + (y + r * 0.707) + ' L' + (x + r * 0.707) + ' ' + (y - r * 0.707) +
                ' M' + (x - r * 0.707) + ' ' + (y - r * 0.707) + ' L' + (x + r * 0.707) + ' ' + (y + r * 0.707);
        }
        if (dotShape === 'barLegend') {
            var p1 = (x - 1.5 * r) + ' ' + (y + r);
            var p2 = (x + 1.5 * r) + ' ' + (y + r);
            var p3 = (x + 1.5 * r) + ' ' + (y - 2 * r);
            var p4 = (x + 0.5 * r) + ' ' + (y - 2 * r);
            var p5 = (x + 0.5 * r) + ' ' + (y - 3 * r);
            var p6 = (x - 0.5 * r) + ' ' + (y - 3 * r);
            var p7 = (x - 0.5 * r) + ' ' + (y - 1 * r);
            var p8 = (x - 1.5 * r) + ' ' + (y - 1 * r);
            return 'M' + p1 + ' L' + p2 + ' L' + p3 + ' L' + p4 + ' L' + p5 + ' L' + p6 + ' L' + p7 + ' L' + p8 + 'Z';
        }
        if (dotShape === 'square') {
            r = r * 0.8;
            return 'M' + (x - r) + ' ' + (y - r) + ' L' + (x + r) + ' ' +
                (y - r) + ' L' + (x + r) + ' ' + (y + r) + ' L' + (x - r) + ' ' + (y + r) + 'Z';
        }

        if (dotShape === 'diamond') {
            return 'M' + (x - r) + ' ' + (y) + ' L' + (x) + ' ' + (y - r) + ' L' +
                (x + r) + ' ' + (y) + ' L' + (x) + ' ' + (y + r) + 'Z';
        }

        if (dotShape === 'rectangle') {
            return 'M' + (x - r) + ' ' + (y - 0.7 * r) + ' L' + (x + r) + ' ' + (y - 0.7 * r) + ' L' +
                (x + r) + ' ' + (y + 0.7 * r) + ' L' + (x - r) + ' ' + (y + 0.7 * r) + 'Z';
        }

        if (dotShape === 'triangleLegend') {
            r = r * 1.2;
            y = y + 0.2 * r;
            return 'M' + (x - 0.866 * r) + ' ' + (y + 0.5 * r) + ' L' + (x + 0.866 * r) + ' '
                + (y + 0.5 * r) + ' L' + (x) + ' ' + (y - r) + 'Z';
        }
        ...
        if (dotShape === 'star') {
            r = 1.5 * r;
            y = y + r / 3.7;
            return 'M' + x + ' ' + (y - r * 1.1) + ' L' + (x - 0.81 * r * 0.75) + ' ' + (y + 0.59 * r * 0.75) + ' L'
                + (x + 0.81 * r) + ' ' + (y - 0.59 * r) + ' L' + (x - 0.81 * r) + ' ' + (y - 0.59 * r) + ' L'
                + (x + 0.81 * r * 0.75) + ' ' + (y + 0.59 * r * 0.75) + 'Z';
        }
    }

    ChartInternal.prototype.updatePointShape = function (cx, cy) {
        var $$ = this;
        var mainCircle = $$.main
        ...
        var mainCircleEnter = mainCircle
            ...
            .attr('d', function (d) {
                ...
                var y = cy(d);
                var r = $$.pointR.bind($$)(d);
                return $$.getDotPathPoints(x, y, r, $$.pointType(d));
            }).attr('cx', cx).attr('cy', cy)
            .style('fill', $$.isStanfordGraphType() ? $$.getStanfordPointColor.bind($$) : $$.color)
            .style('stroke', $$.isStanfordGraphType() ? $$.getStanfordPointColor.bind($$) : $$.color);
        $$.mainCircle = mainCircleEnter
            .merge(mainCircle)
            .style('opacity', $$.isStanfordGraphType() ? 1 : $$.initialOpacityForPoints.bind($$));
        mainCircle.exit().style('opacity', 0);
    };
    ChartInternal.prototype.redrawPointShape = function (cx, cy, withTransition, transition) {
        var $$ = this, selectedPoints = $$.main.selectAll('.' + CLASS.selectedPoint);
        return [
            (withTransition ? $$.mainCircle.transition(transition) : $$.mainCircle)
                .style('opacity', this.opacityForPoints.bind($$))
                .style('fill', $$.isStanfordGraphType() ? $$.getStanfordPointColor.bind($$) : $$.color)
                .style('stroke', $$.isStanfordGraphType() ? $$.getStanfordPointColor.bind($$) : $$.color)
                .attr('d', function (d) {
                    var x = cx(d);
                    ...
                    var r = $$.pointR.bind($$)(d);
                    return $$.getDotPathPoints(x, y, r, $$.pointType(d));
                }).attr('cx', cx).attr('cy', cy).style('stroke-width', '2px'),
            (withTransition ? selectedPoints.transition(transition) : selectedPoints)
                .attr('d', function (d) {
                    var x = cx(d);
                    var y = cy(d);
                    var r = $$.pointR.bind($$)(d);
    ...
    ChartInternal.prototype.pointX = function (d) {
        return d.x || d.x === 0 ? this.x(d.x) : null;
    };
    ChartInternal.prototype.updatePointY = function () {
    ...
    ChartInternal.prototype.getDotPoints = function (i, id) {
        var $$ = this;
        return (id
            ? $$.main.selectAll('.' + CLASS.circles + $$.getTargetSelectorSuffix(id))
            : $$.main).selectAll('.' + CLASS.circle + (isValue(i) ? '-' + i : ''));
    };
    ChartInternal.prototype.expandPointShapes = function (i, id, reset) {
        ...
        $$.getDotPoints(i, id)
            ...
            .attr('d', function (d) {
                var x = $$.pointX(d);
                var y = $$.pointY(d);
                return $$.getDotPathPoints(x, y, r(d), $$.pointType(d));
            });
    };
    ChartInternal.prototype.unexpandPointShapes = function (i) {
    ...
    ChartInternal.prototype.pointType = function (d) {
        var $$ = this, config = $$.config;
        return $$.isStepType(d)
            ? 0
            : isFunction(config.point_type)
                ? config.point_type(d)
                : config.point_type;
    };

    ChartInternal.prototype.lineStyle = function (d) {
        var $$ = this, config = $$.config;
        return isFunction(config.data_lineStyle)
            ? config.data_lineStyle(d)
            : config.data_lineStyle;
    };

    ChartInternal.prototype.lineWidth = function (d) {
        var $$ = this, config = $$.config;
        return isFunction(config.data_lineWidth)
            ? config.data_lineWidth(d)
            : config.data_lineWidth;
    };
    ...
    ChartInternal.prototype.legendShape = function (d) {
        var res = { shape: '', r: '' };
        if (this.isBarType(d)) {
            res.shape = 'rectangle';
            res.r = this.config.legend_item_tile_dotSize * 1.5;
        } else if (this.isArcType(d)) {
            res.shape = 'circle';
            res.r = this.config.legend_item_tile_dotSize;
        } else {
            res.shape = this.pointType({ id: d }) === 'triangle' ? 'triangleLegend' : this.pointType({ id: d });
            res.r = this.config.legend_item_tile_dotSize;
        }
        return res;
